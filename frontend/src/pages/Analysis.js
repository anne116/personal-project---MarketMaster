...
const Analysis = () => {
  ...
  const fetchStatistics = async () => {
    try {
      const response = await fetch(
        `/fetch_statistics?keyword=${encodeURIComponent(keyword)}`,
      );
  ...
  const fetchSuggestedTitle = async() => {
    try {
        const titleResponse = await fetch(`/suggested_title?keyword=${encodeURIComponent(keyword)}`)
    ...
    datasets: [
      {
        ...
        data: stats && stats.price_list && stats.rating_list
      ...
      <Box maxW='1200px' mx='auto'>
        ...
        {stats ? (
          <VStack spacing={4} align='stretch'>
            ...
            <Grid templateColumns='repeat(2, 1fr)' gap={6}>
              <GridItem>
                <Box p={4} bg={theme.colors.brand[700]} boxShadow='md' borderRadius='md' height={`${chartHeight}px`}>
                  <Bar data={priceRangeData} options={{...chartOptions, plugins: {...chartOptions.plugins, title: {...chartOptions.plugins.title, text: 'Price Range Distribution ($US)' } } }} />
                </Box>
              </GridItem>
              <GridItem>
                <Box p={4} bg={theme.colors.brand[700]} boxShadow='md' borderRadius='md' height={`${chartHeight}px`}>
                  <Bar data={ratingDistributionData} options={{...chartOptions, plugins: {...chartOptions.plugins, title: {...chartOptions.plugins.title, text: 'Rating Distribution' } } }} />
                </Box>
              </GridItem>
            </Grid>
            ...
            <Grid templateColumns='repeat(2, 1fr)' gap={6}>
              <GridItem>
                <Box p={4} bg={theme.colors.brand[700]} boxShadow='md' borderRadius='md' height={`${chartHeight}px`}>
                  <Bar data={reviewRangeData} options={{...chartOptions, plugins: {...chartOptions.plugins, title: {...chartOptions.plugins.title, text: 'Review Distribution' } } }} />
                </Box>
              </GridItem>
              <GridItem>
                <Box p={4} bg={theme.colors.brand[700]} boxShadow='md' borderRadius='md' height={`${chartHeight}px`}>
                  <Pie data={productDistributionByRatingData} options={{...pieChartOptions, plugins: {...pieChartOptions.plugins, title: {...chartOptions.plugins.title, text: 'Product Distribution By Rating' } } }} />
                </Box>
              </GridItem>
            </Grid>
            ...
            <Grid templateColumns='repeat(2, 1fr)' gap={6}>
              <GridItem>
                <Box p={4} bg={theme.colors.brand[700]} boxShadow='md' borderRadius='md' height={`${chartHeight}px`}>
                  <Scatter data={priceVsReviewsData} options={scatterChartOptions('Price vs Reviews', 'Number of Reviews')} />
                </Box>
              </GridItem>
              <GridItem>
                <Box p={4} bg={theme.colors.brand[700]} boxShadow='md' borderRadius='md' height={`${chartHeight}px`}>
                  <Scatter data={priceVsRatingData} options={scatterChartOptions('Price vs Rating', 'Rating')} />
                </Box>
              </GridItem>
            </Grid>
